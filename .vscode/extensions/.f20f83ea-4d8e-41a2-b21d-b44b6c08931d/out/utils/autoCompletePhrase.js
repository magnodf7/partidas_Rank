		...
		if (text.length) {
			...
			if (lastLine.includes('?')==false && text.length>10){
				...
				if (premium_status){
					...
					let url_request = ''
					try{
						const response = await (0, node_fetch_1.default)('https://www.useblackbox.io/suggestRequest', {
							method: 'POST',
							body: JSON.stringify({
						...
						try{
							const response = await (0, node_fetch_1.default)(url_request, {
								method: 'POST',
								body: JSON.stringify({
									data: [text]
								}),
							...
							result = await response.json();
							try{
								sentence = result['data'][0]
								sentence = sentence.trim();
								sentence = filterOutput(sentence)

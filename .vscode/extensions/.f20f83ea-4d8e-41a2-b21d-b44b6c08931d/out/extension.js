...
function activate(_) {
			...
			chatPanel.webview.onDidReceiveMessage(async (data) => { 
				if (data.command === "sendMessage") {
					...
					try {
						...
						if (is_stream && is_excess_context_size == false && webSocket.readyState == 1 && is_demo == false){// Stream Response
							...
							full_answer = ''
							webSocket.send(JSON.stringify({command: 'generate', content: data.allMessages, userId: userId}));
						}else{
							const response = await (0, node_fetch_1.default)(
  ...
  function getFolderTimestamp(folderName) {
	...
	vscode.commands.registerCommand("extension.getSuggest", () => {
		webSocket.send(JSON.stringify({command:"editorautocomplete", context: getLinesAbove(), userId: userId}))
